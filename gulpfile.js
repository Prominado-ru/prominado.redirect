...
const getTags = () => {
  return new Promise(resolve => {
    git.exec(
        {args: 'tag --sort=-creatordate --format=\'%(creatordate:rfc)%09%(refname)\''},
        (error, output) => {
          if (error) {
            throw error;
          }
...
const parseVersions = log => {
  ...
  return tags.map((tag) => {
    ...
    if (match.length > 1) {
      ...
      if (!isNumeric(versionArray[0]) || !isNumeric(versionArray[1]) ||
          !isNumeric(versionArray[2])) {
        return null;
      }
...
const getVersionFolderName = () => {
  ...
  if (lastVersion.version === previousVersion.version && os.platform() ===
      'darwin') {
    return '.' + folderName;
  }
...
gulp.task('version', () => {
  const version = getVersionFolderName();
  const fileContent = createVersionFileContent(lastVersion.version,
      lastVersion.date);

  return fs.writeFile(path.join(buildFolder, version, 'install', 'version.php'),
      fileContent, (error) => {
        console.log(error);
      });
});
...
gulp.task('diff', (callback) => {
  git.exec({args: `diff ${previousVersion.version} --name-only`},
      (error, output) => {
        if (error) {
          callback(error);
        }
...
gulp.task('build_last_version', (callback) => {
  getTags().then(function(output) {
    ...
    sequence('clean', 'move', 'version', 'encode', 'archive', 'dist', 'clean',
        callback);
  }).catch((error) => {
    console.log(error);
  });
...
gulp.task('build_update', (callback) => {
  getTags().then(function(output) {
    ...
    sequence('clean', 'diff', 'version', 'encode', 'archive', 'dist', 'clean',
        callback);
  }).catch((error) => {
    console.log(error);
  });
